...
function Tableview() {
  ...
  const [sortData, setSortData] = useState({ key: 'id', direction: 'ascending' });
  ...
  useEffect(() => {
    ...
    // Sort data based on sortData
    const sortedData = filteredData.sort((a, b) => {
      const aValue = a[sortData.key];
      const bValue = b[sortData.key];

      if (aValue < bValue) return sortData.direction === 'ascending' ? -1 : 1;
      if (aValue > bValue) return sortData.direction === 'ascending' ? 1 : -1;
      return 0;
    });
  ...
  const handleSort = (key) => {
    const direction = (sortData.key === key && sortData.direction === 'ascending') ? 'descending' : 'ascending';
      ...
      <table border="2">
        <thead>
          <tr>
            ...
            <th onClick={() => handleSort('isActive')}>Is Active</th>
            <th onClick={() => handleSort('salary')}>Salary</th>
            <th onClick={() => handleSort('department')}>Department</th>
            <th onClick={() => handleSort('projectsCompleted')}>Projects Completed</th>
            <th onClick={() => handleSort('lastLogin')}>Last Login</th>
            <th onClick={() => handleSort('accessLevel')}>Access Level</th>
          </tr>
        </thead>
        <tbody>
          {data.map(r => (
            <tr key={r.id}>
              ...
              <td>{r.salary}</td>
              <td>{r.department}</td>
              <td>{r.projectsCompleted}</td>
              <td>{new Date(r.lastLogin).toLocaleDateString()}</td>
              <td>{r.accessLevel}</td>
            </tr>
          ))}
        </tbody>
